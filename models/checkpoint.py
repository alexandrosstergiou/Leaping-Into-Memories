...
import models.distributed as du
#import slowfast.utils.logging as logging
from models.c2_model_loading import get_name_convert_func
...
def load_checkpoint(
    ...
    """
    print("Loading network weights from {}.".format(path_to_checkpoint))
    ...
    if convert_from_caffe2:
        ...
        # spacial case for swin
        if "state_dict" in checkpoint.keys():
            checkpoint["model_state"] = checkpoint["state_dict"]
            new_dict = {}
            new_dict["model_state"] = {}
            for key in checkpoint["model_state"].keys():
                if "backbone" in key:
                    new_k = key.split('backbone.')[-1]      
                    new_dict["model_state"][new_k]=checkpoint["model_state"][key]
                else:
                    new_dict["model_state"][key]=checkpoint["model_state"][key]
            new_dict = OrderedDict(new_dict)
            checkpoint =  new_dict
             
        # special case for uniformer
        elif "model_state" not in checkpoint.keys():
        ...
        if inflation:
            ...
            if not_used_layers:
                for k in not_used_layers:
                    print("Network weights {} not used.".format(k))
...
def load_train_checkpoint(cfg, model, optimizer, scaler=None):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR, cfg.TASK)
        print("Load from last checkpoint, {}.".format(last_checkpoint))
